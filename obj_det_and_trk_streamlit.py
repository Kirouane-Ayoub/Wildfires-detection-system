...
def draw_boxes(img, bbox, identities=None, categories=None, names=None,offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        count_list.append(id)
        #data = (int((box[0]+box[2])/2),(int((box[1]+box[3])/2)))
        #color = compute_color_for_labels(id)
        color = [0, 255, 0]
        label = f"Num:{id} of {cat}"
        #label = str(cat)
        (w, h), _ = cv2.getTextSize(cat, cv2.LINE_AA, 1, 1)
        cv2.rectangle(img, (x1, y1), (x2, y2), color, 2)
        cv2.rectangle(img, (x2, y2 - 30), (x2 + w, y2), color, -1)
        cv2.putText(img, label, (x2, y2 - 5),cv2.LINE_AA, 1, [225, 255, 225], 2)
        cv2.putText(img,f"Total number of detections : {str(max(count_list))}", (20,25), cv2.LINE_AA,1, color ,2)
...
@torch.no_grad()
def detect(weights=ROOT / 'smoke.pt',
        source=ROOT / '', 
